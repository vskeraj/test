    ...
    >
      <Link href={`/products/${book.id}`}>
        <div className={`relative h-48 bg-gradient-to-br ${coverColor} flex items-center justify-center overflow-hidden`}>
          ...
          {/* Spine + page-edge detail so covers read as real books */}
          <div className="absolute inset-y-0 left-0 w-1.5 bg-black/25 shadow-[inset_-1px_0_2px_rgba(255,255,255,0.12)]" />
          ...
          <span className="relative text-3xl font-display text-white/90 text-center px-4 leading-tight drop-shadow-[0_2px_8px_rgba(0,0,0,0.4)]">{book.title}</span>
        </div>
      </Link>
      <div className="p-4">
